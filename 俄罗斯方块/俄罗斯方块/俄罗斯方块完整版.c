...
void init()
{
	...
	outtextxy(460, 360, L"QQ群:879098023");
	outtextxy(460, 400, L"最终解释权归梦凡所有");
...
void NewBlock()
{
	...
	//wchar_t arr[100];
	//wsprintf(arr, L"[%d,%d],[%d,%d]", g_NextBlock.x, g_NextBlock.y, g_CurBlock.x, g_CurBlock.y);
	//OutputDebugString(arr);
	setfillcolor(BLACK);
	solidrectangle((WIDTH + 1)*PIX + 1, 1, (WIDTH + 6)*PIX - 1, 6 * PIX - 1);
