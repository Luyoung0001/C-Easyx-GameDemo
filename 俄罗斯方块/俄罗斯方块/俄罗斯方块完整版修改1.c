...
void NewGame();		//开始新游戏
void NewBlock();	//得到新方块
void DrawBlock(BLOCKINFO block, FLAG flag = SHOW);	//画方块
void DrawUnit(int x,int y,COLORREF color, FLAG flag);	//画方块的每一个小方块
void Select();		//功能选择 ↑ ↓ ← →
void OnRight();		//左移
void OnLeft();		//右移
void OnDown();		//下降
bool CheckBlock(BLOCKINFO block);	//检查方块位置是否合法
void OnSink();		//下沉
void OnRotate();	//旋转
void Rotate(int dx);//旋转
void Stop();		//暂停
void Restart();		//从新开始
...
void init()
{
	...
	outtextxy(460, 320, L"VX公众号:编程学习基地");
	outtextxy(460, 360, L"QQ交流群:879098023");
	outtextxy(460, 440, L"最终解释权归梦凡所有");

	g_score = 0;
	//画两个矩形
	setorigin(220, 20);		//设置原点
	setlinecolor(WHITE);
...
void NewBlock()
{
	...
	//0x4444&0x000F
	//0x0004 ! = 0	说明十六宫格最后一行不为为空，表示g_CurBlock的图形可以在屏幕上显示最后一行
	while ((c & 0x000F) == 0)
	{
		g_CurBlock.y--;
		c >>= 4;
...
void DrawBlock(BLOCKINFO block, FLAG flag)
{
	...
	{
		//1000 0000 0000 0000
		//0100 0100 0100 0100	->0x4444 I型方块  ->b左移一位 1000 1000 1000 1000 
		//1000 0000 0000 0000 & 1000 1000 1000 1000 ->为真绘制第二个方格 ->循环上面操作
		if (b & 0x8000)
		{
			x = block.x + i % 4;	
			y = block.y - i / 4;
...
{
	int left = x * PIX ;			// + 1 是为了防止擦除边框线
	int right = (x + 1) * PIX - 1;	// -1 是为了防止擦除边框线
...
void OnSink()
{
	...
	for (y = g_CurBlock.y; y >= max(g_CurBlock.y - 3, 0); y--)
	{
		...
		if (i == WIDTH)
		{
			remove |= (1 << (g_CurBlock.y - y));//100 | 10 --11 111  1111
